...
def main(adjective_file, verb_file):
	...
	f = open("results_multi_suggest.txt", "w")
	vowels = ["a", "e", "i", "o", "u"]
	google = Google()
	pool = mp.Pool(processes=6) # multiprocessing

	# start reading txt files line by line and extract adjectives/verbs
	for i in range(max(len(adj_lines), len(verb_lines))):
		results = []

		# find google recommanded searches for adjectives
		if i < len(adj_lines):
			...
			for suggestion in suggestions:
				num_results = pool.apply_async(google.get_num_searches, args=(suggestion,))
				results.append((suggestion, num_results))
		...
		# find google recommanded searches for verbs
		if i < len(verb_lines):
			verb = verb_lines[i].strip().split("\t")[0]

			# simple present tense
			query = '''"I {} because "'''.format(verb)
			suggestions = google.get_recommanded_query(query)
			for suggestion in suggestions:
				num_results = pool.apply_async(google.get_num_searches, args=(suggestion,))
				results.append((suggestion, num_results))

			query = '''"You {} because "'''.format(verb)
			suggestions = google.get_recommanded_query(query)
			for suggestion in suggestions:
				num_results = pool.apply_async(google.get_num_searches, args=(suggestion,))
				results.append((suggestion, num_results))

			query = '''"They {} because "'''.format(verb)
			suggestions = google.get_recommanded_query(query)
			for suggestion in suggestions:
				num_results = pool.apply_async(google.get_num_searches, args=(suggestion,))
				results.append((suggestion, num_results))

			query = '''"We {} because"'''.format(verb)	
			suggestions = google.get_recommanded_query(query)
			for suggestion in suggestions:
				num_results = pool.apply_async(google.get_num_searches, args=(suggestion,))
				results.append((suggestion, num_results))
			...
			query = '''"She {} because "'''.format(verb2)	
			suggestions = google.get_recommanded_query(query)
			for suggestion in suggestions:
				num_results = pool.apply_async(google.get_num_searches, args=(suggestion,))
				results.append((suggestion, num_results))

			query = '''"He {} because "'''.format(verb2)	
			suggestions = google.get_recommanded_query(query)
			for suggestion in suggestions:
				num_results = pool.apply_async(google.get_num_searches, args=(suggestion,))
				results.append((suggestion, num_results))
		...
		for result in results:
			num = result[1].get()
			f.write(result[0] + "\t" + str(num) + "\n")
			print(result[0] + "\t" + str(num)) 			
